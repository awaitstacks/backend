...
export const cancelBookingController = async (req, res) => {
  try {
    ...
    if (advancePaid && balanceAmount === 0 && balancePaidFlag) {
      ...
      // 5. Total Cancellation Amount — GV + IRCTC + Remarks only.
      // (bookingBalanceManagementAmount is already 0 in this case,
      // so no behavioural change here.)
      const totalCancellationAmount = Number(
        (
          gvCancellationAmount +
          irctcCancellationAmount +
    ...
    // CASE 2: Advance Paid, Balance > 0, Unpaid, More Than One Traveller
      ...
      // G. Positive admin remarks (extra charges) — still calculated
      // and stored (still drives preBalanceAmount / updatedBalance /
      // refundAmount below, unchanged), just no longer folded into
      // totalCancellationAmount.
      const bookingBalanceManagementAmount = positiveAdminSum;

      // 2. Pre-Balance Amount = Active Cost + All Deductions (unchanged
      // — still includes bookingBalanceManagementAmount, this drives
      // the actual balance settlement math below)
      const preBalanceAmount = Number(
        (
          activeTravellersCost +
          gvCancellationAmount +
      ...
      cancellationDoc = new cancellationModel({
        ...
        remarksAmount: remarksAmountFinal,
        bookingBalanceManagementAmount,
        // Total Cancellation Amount — GV + IRCTC + Remarks only
        // (bookingBalanceManagementAmount excluded from this field;
        // it's still tracked separately above and still drives
        // preBalanceAmount/updatedBalance/refundAmount as before).
        totalCancellationAmount: Number(
          (
            gvCancellationAmount +
            irctcCancellationAmount +
    ...
    // CASE 3: Advance Paid, Balance > 0, Unpaid, Only One Traveller
      ...
      // G. Positive Admin Remarks — unchanged, still drives
      // preBalanceAmount/updatedBalance/refundAmount below.
      const bookingBalanceManagementAmount = positiveAdminSum;

      // 2. Pre Balance Amount (unchanged)
      const preBalanceAmount = Number(
        (
          gvCancellationAmount +
          irctcCancellationAmount +
      ...
      // 3. Updated Balance / Refund Amount (unchanged)
      const updatedBalanceRaw = Number(
        (preBalanceAmount - netAmountPaid).toFixed(2),
      );
      const updatedBalance = updatedBalanceRaw >= 0 ? updatedBalanceRaw : 0;
      ...
      cancellationDoc = new cancellationModel({
        ...
        bookingBalanceManagementAmount,
        // Total Cancellation Amount — GV + IRCTC + Remarks only.
        // Previously this field mirrored preBalanceAmount (which
        // includes bookingBalanceManagementAmount); preBalanceAmount
        // itself is untouched and still drives updatedBalance/refund.
        totalCancellationAmount: Number(
          (
            gvCancellationAmount +
            irctcCancellationAmount +
    ...
    // CASE 4: Advance Paid, Balance > 0, Paid and Verified, Only One Traveller
      ...
      // B. Positive Admin Remarks — unchanged, still drives
      // updatedBalance/refundAmount below.
      const bookingBalanceManagementAmount = positiveAdminSum;
      ...
      // Updated Balance (unchanged — still subtracts
      // bookingBalanceManagementAmount as before)
      const updatedBalanceRaw = Number(
        (
          preBalanceAmount -
          bookingBalanceManagementAmount -
      ...
      cancellationDoc = new cancellationModel({
        ...
        // Total Cancellation Amount — GV + IRCTC + Remarks only
        // (bookingBalanceManagementAmount excluded from this field;
        // updatedBalance/refundAmount above still account for it).
        totalCancellationAmount: Number(
          (
            gvCancellationAmount +
            irctcCancellationAmount +
    ...
    // CASE 5: Advance Paid, Balance > 0, Paid and Verified, More Than One Traveller
      ...
      // B. Positive Admin Remarks — unchanged, still drives
      // updatedBalance/refundAmount (via totalDeduction) below.
      const bookingBalanceManagementAmount = hasApprovedCancellation
        ? 0
        : positiveAdminSum;
      ...
      // Updated Balance (unchanged — totalDeduction still includes
      // bookingBalanceManagementAmount when applicable, exactly as
      // before; this is what settles the balance, separate from the
      // totalCancellationAmount field stored below)
      const totalDeduction = hasApprovedCancellation
        ? gvCancellationAmount + irctcCancellationAmount + remarksAmountFinal
        : bookingBalanceManagementAmount +
          gvCancellationAmount +
      ...
      cancellationDoc = new cancellationModel({
        ...
        // Total Cancellation Amount — GV + IRCTC + Remarks only
        // (bookingBalanceManagementAmount excluded from this field;
        // totalDeduction above — used for updatedBalance/refundAmount —
        // is untouched and still accounts for it when applicable).
        totalCancellationAmount: Number(
          (
            gvCancellationAmount +
            irctcCancellationAmount +
